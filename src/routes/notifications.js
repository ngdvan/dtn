...
function createNotificationRoutes({ db, auth, asyncRoute }) {
  ...
  router.get('/api/notifications', auth, asyncRoute(async (req, res) => {
    ...
    const [[notifications], [counts]] = await Promise.all([
      db.execute('SELECT id,kind,title,body,url,seen_at,created_at,expires_at FROM notifications WHERE user_id=? AND expires_at>NOW() ORDER BY created_at DESC LIMIT 20', [req.session.user.id]),
